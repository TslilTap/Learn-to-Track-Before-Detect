...
from Configuration.motion_model_config import motion_model_param
...
class MotionModel:
    ...
    """
    def __init__(self, motion_model_param, epsilon = 1e-10):
        self.env = motion_model_param['environment'] # model of range and velocity bins
        self.sigma_r = torch.tensor(motion_model_param['sigma_r']) # range transition variance
        self.sigma_d = torch.tensor(motion_model_param['sigma_d']) # doppler velocity transition variance
        self.T = motion_model_param['T'] # time [sec] passed between each sample
...
motion_model = MotionModel(motion_model_param=motion_model_param)